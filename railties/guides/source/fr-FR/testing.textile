h2. Un guide du test des applications Rails

Ce guide couvre les dispositifs inclus dans Rails pour le test de vos applications. A traver ce guide, vous allez :

* comprendre la terminologie de Rails pour les tests,
* écrire des tests unitaires, fonctionnels et d'intégration pour vos applications,
* identifier d'autres approches courantes du test ainsi que des plugins dédiés.

Ce guide ne vas pas vous apprendre à écrire une application Rails ; il suppose une familiarité de base avec la vision de Rails.

endprologue.

h3. Pourquoi écrire des tests pour vos applications Rails ?

* Rails rend très simple l'écriture de tests. Il commence par produire le squelette de vos tests, en arrière-plan, alors que vous créez vos modèles et vos contrôleurs,
* Simplement en exécutant vos tests Rails vou spouvez vous assurez que votre application respecte toujours les fonctionnalités désirées même après un refactoring majeur du code.
* Les tests de Rails peuvent également simuler des requètes du navigateur et ainsi tester votre application sans avoir à le faire à travers le navigateur.

h3. Introduction au test

Le support des tests est inscrit dans l'ADN de Rails depuis le départ. Il n'y a pas eut une grande révélation du type "Allez, il faut qu'on supporte les tests parce que ça vient de sortir et que c'est supper".

A peu près toutes les applications Rails interagissent lourdement avec une base de données ; vos tests auront donc également besoin d'interagir avec une base de données. Pour écrire des tests efficaces, vous alez devoir comprendre comment créer cette base et l'initialiser avec des données exemples.

h4. Les 3 environnements

Chaque application Rails que vous construisez a 3 cotés : un coté pour la production, un coté pour le développement et un coté pour le test.

Cette distinction apparaît nettement dans le fichier +config/database.yml+. Ce fichie de configuration YAML a 3 sections différentes définissant 3 configurations différentes de bases de données :

* production
* development
* test

Cela vous permet d'initialiser vos données et d'agir sur elles à travers les tests sans faire courir de risques à vos données de production dans cette environnement.

Par exemple, supposez que vous ayez à tester votre nouvelle fonction +supprime_un_utilisateur_et_tout_ce_qui_va_avec+. N'aimeriez-vous pas exécuter vos tests dans un environnement où la destruction de données importe peu ?

S'il vous arive de détruire la base de test (et cela va arriver, faites moi confiance), vous pouvez la reconstruire de zéro conformément à la base de développement. Tout simplement en exécutant +rake db:test:prepare+.
