...
Ce guide présente différente façons d'obtenir des des données d'une base de données avec d'Active Record. En vous reportant à ce guide, vous pourrez :
...
Active Record effectue des requètes sur une base de données pour vous et est compatible avec la plupart des moteurs (MySQL, PostgreSQL et SQLite pour en nommer quelques-uns). Indépendamment du moteur utilisé, le format des méthodes Active Record reste le même.

h3. Obtenir des objets depuis la base de données

Pour obtenir des objets de la base de données, Active Record fournit plusieurs méthodes "finder" (de recherche). Chaque méthode accepte des arguments pour effectuer des requètes sur la base sans écrire directement du SQL.
...
Le fonctionnement d'une opération comme <tt>Model.find(options)</tt>
peut-être résumé par :

* convertir les options fournies dans leur équivalent SQL,
* exécuter la requète SQL et obtenir les resultats correspondants depuis la base,
...
<tt>Model.first</tt> trouve le premier enregistrement correspndant aux options. Ainsi :
...
<tt>Model.last</tt> trouve le dernier enregistrement correspndant aux options. Ainsi :
...
Parfois vous devez parcourir un grand nombre d'enregitrements. Par exemple pour envoyer une lettre à tous les utilisateurs, pour exporter des données, etc...
...
Pour parcourir efficacement une grande table, Active Record fournit une méthode de recherhe par paquets appelée +find_each+:
...
Derrière le rideau +find_each+ récupère les rangées par parquets de
+1000+ et les fournit un par un. La taille de ces paquets est
configurable avec l'option +:batch_size+.
...
Les enregistrements sont récupérés en ordre ascendant sur la clef primaire, qui doit être un entier. L'option +:start+ permet de configurer le premier ID de la séquence si vous ne souhaitez pas récupérer le premier. Ca peut être utile par exemple pour pouvoir reprendre un traitement interrompu s'il sauvegarde le dernier ID traité comme point de contrôle.
...
+find_each+ accept les mêmes options qu'un +find+ standard. Cependant, +:order+ et +:limit+ sont nécessaires pour des besoins interns et donc ne peuvent pas être passés explicitement.

h5. +find_in_batches+

Vous pouvez aussi travailler per blocs plutôt que par rangée avec +find_in_batches+. Cette méthode est semblable à +find_each+, mais fournit des tableaux de modèles à la place :
...
La méthode +find+ vous permet de spécifier des conditions pour limiter les enregistrements retournés, correspondant à la partie +WHERE+ d'une requète SQL. Les conditions peuvent être spécifier en tant que string, array ou hash.
...
Active Record utilise le premier élément des conditions et les
éléments supplémentaires y remplacent les points d'interrogation +?+.

Pour spécifier deux conditions, vous faîtes :
